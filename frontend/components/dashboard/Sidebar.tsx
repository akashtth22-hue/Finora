...
export default function Sidebar({
  ...
  return (
    <>
      ...
      <aside className="group/sidebar relative hidden w-20 shrink-0 lg:flex">
        <div className="absolute inset-y-0 left-0 z-40 flex w-20 flex-col overflow-hidden border-r border-gray-200 bg-white shadow-sm transition-all duration-300 ease-out group-hover/sidebar:w-64">
          ...
          <div className="flex h-[105px] shrink-0 items-start border-b border-gray-200 px-5 pt-5">
            <div className="flex min-w-[224px] items-center gap-4">
              ...
              {/* Expanded Branding */}
              <div className="-translate-x-2 min-w-0 opacity-0 transition-all duration-300 delay-75 group-hover/sidebar:translate-x-0 group-hover/sidebar:opacity-100">
            ...
            {menuItems.map((item, index) => {
              ...
              return (
                <Link
                  key={item.title}
                  href={item.href}
                  className="group/item flex h-12 min-w-[224px] items-center gap-4 rounded-xl px-3 text-gray-700 transition-all duration-200 hover:bg-purple-50 hover:text-purple-600"
                >
                  {/* Icon */}
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl transition-transform duration-200 group-hover/item:scale-105">
                    <Icon size={22} />
                  </div>

                  {/* Label */}
                  <span
                    className="-translate-x-2 whitespace-nowrap font-medium opacity-0 transition-all duration-300 group-hover/sidebar:translate-x-0 group-hover/sidebar:opacity-100"
                    style={{
              ...
              {menuItems.map((item) => {
                ...
                return (
                  <Link
                    ...
                    href={item.href}
                    onClick={() => setMobileMenuOpen(false)}
                    className="flex items-center gap-4 rounded-xl px-4 py-3 text-gray-700 transition hover:bg-purple-50 hover:text-purple-600"
                  >
                    <Icon size={21} />
